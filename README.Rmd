...
The goal of **creditools** is to put the computational power of an entire risk analytics team into a single, scalable R package. It provides a flexible framework for mathematically simulating and optimizing credit policies, now featuring **Analytical Reweighting** for deterministic, noise-free analysis.
...
*   **⚡ Analytical vs. Stochastic:** Choose between row-by-row sampling (`stochastic`) or expected-value calculation (`analytical`). The latter provides instant, deterministic results for tradeoff analysis.
*   **🎯 Optimal Efficient Frontier:** Automatically discover the exact cutoffs needed to hit a target default rate while maximizing approval volume.
*   **🧪 Surgical Stress Testing:** Inject custom stress scenarios for "swap-ins" (newly approved clients) to account for unsupervised risk.
*   **🌊 Multi-Stage Funnels:** Model complex chains of decisions (Filters → Cutoffs → Conversion → Anti-Fraud) using a clean, pipeable syntax.
...
If you have a historical dataset, you can run a full policy impact analysis in seconds. Using the new `analytical` method, we calculate the **expected impact** without sampling noise.
...
Most credit policies involve more than just a score. Use `add_stage()` to build a realistic acquisition funnel.
...
Find the "Efficient Frontier" of your policy by varying cutoffs and stress factors.
...
Use `find_risk_groups()` to bin multiple scores into a stable Risk Matrix, ensuring that your risk ratings remain consistent across historical vintages.
...
Check the vignettes for deep dives into real-world use cases:
- [Case Study: Used Vehicles](vignettes/case-study-used-vehicles.html)
