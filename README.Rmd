...
# Ensure we load the local development version
devtools::load_all(".")
```
...
4.  **`simulate_from_data()`**: The high-level "Data-to-Insights" wrapper. Perfect for analysts who already have a flat table with historical outcomes and just want to backtest a single cutoff with Ward-clustering and PD stress in one command.

## Quick Start: Data-to-Insights

If you already have a historical dataset with scores and outcomes (Default, Approval, Hiring), you don't need to define complex stages. You can get a complete tradeoff and RBP matrix in one line:
